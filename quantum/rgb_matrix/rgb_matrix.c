...
__attribute__((weak)) uint8_t find_rf_led_index(uint8_t index) {
    return index;
}
bool g_remap_rf_led = false;

void rgb_matrix_set_color(int index, uint8_t red, uint8_t green, uint8_t blue) {
    if (g_remap_rf_led && index >= 0 && index <= 4) {
        index = find_rf_led_index((uint8_t)index);
    }
